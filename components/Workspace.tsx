    ...
    <div className="flex flex-col md:flex-row h-full overflow-hidden relative">
      ...
      {itemToPublish && (
          <div className="absolute inset-0 z-50 bg-black/80 flex items-center justify-center p-4 backdrop-blur-sm">
              <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-850">
                      <h3 className="font-bold text-white flex items-center gap-2">
                          <Share className="w-4 h-4 text-indigo-400" />
                          {t('workspace.publishModal.title')}
                      </h3>
                      <button onClick={() => setItemToPublish(null)} className="text-gray-400 hover:text-white">
                          <X className="w-5 h-5" />
                      </button>
                  </div>
                  ...
                  <div className="p-6 space-y-4">
                      <div>
                          <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">{t('workspace.publishModal.workTitle')}</label>
                          <input 
                              type="text" 
                              className="w-full bg-gray-950 border border-gray-700 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      ...
                      <div>
                          <label className="block text-xs font-bold text-gray-400 mb-1 uppercase">{t('workspace.publishModal.description')}</label>
                          <textarea 
                              className="w-full bg-gray-950 border border-gray-700 rounded-lg p-3 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none h-24"
                      ...
                      <div>
                          <label className="block text-xs font-bold text-gray-400 mb-2 uppercase">{t('workspace.publishModal.visibility')}</label>
                  ...
                  <div className="p-4 bg-gray-850 flex justify-end gap-3">
                      <button 
      ...
      {/* Left Panel: Controls */}
      <div className={`relative h-full flex flex-col p-6 overflow-y-auto border-r border-gray-800 gap-6 transition-all duration-300 ease-in-out ${showSidebar ? 'w-full md:w-2/3 lg:w-3/4' : 'w-full'}`}>
        
        {/* Toggle Sidebar Button */}
        <div className="absolute top-6 right-6 z-20">
            <button 
                onClick={() => setShowSidebar(!showSidebar)}
                className="p-2 bg-gray-800 hover:bg-gray-700 rounded-lg text-gray-400 hover:text-white transition-colors border border-gray-700 shadow-md"
        ...
        <div className="flex flex-col gap-4 flex-grow">
            <div className="flex flex-col md:flex-row gap-6 h-full">
                ...
                <div className="w-full md:w-1/3 flex flex-col gap-2">
                    ...
                    <div className="relative">
                        ...
                        {localizedActiveVoice ? (
                            <div className="bg-indigo-900/30 border border-indigo-500/50 rounded-xl p-4 flex flex-col gap-3 relative group transition-all hover:bg-indigo-900/40">
                                <div className="flex items-start gap-3">
                                    <img src={localizedActiveVoice.imageUrl} className="w-12 h-12 rounded-lg object-cover" alt="ref" />
                                    <div>
                                        <h4 className="text-white font-bold text-sm">{localizedActiveVoice.title}</h4>
                                        <p className="text-xs text-indigo-300">{t('gallery.by')} {localizedActiveVoice.author}</p>
                                    </div>
                                </div>
                                <div className="flex flex-wrap gap-1 mt-1">
                                    <span className="px-2 py-0.5 rounded-full bg-indigo-900 text-indigo-200 text-[10px] border border-indigo-700">
                                        {t(`gallery.filters.${localizedActiveVoice.category}`)}
                                    </span>
                                </div>
                                <div className="h-1 bg-indigo-500/30 rounded-full w-full overflow-hidden mt-1">
                            ...
                            <button 
                                onClick={() => setIsVoiceDropdownOpen(!isVoiceDropdownOpen)}
                                className="w-full border-2 border-dashed border-gray-700 rounded-xl p-6 flex flex-col items-center justify-center gap-2 text-gray-500 hover:border-indigo-500 hover:bg-gray-800/50 hover:text-indigo-400 transition-all h-40"
                            >
                        ...
                        {/* Dropdown Menu */}
                        {isVoiceDropdownOpen && (
                            <div className="absolute top-full left-0 right-0 mt-2 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl z-20 max-h-80 overflow-y-auto">
                                <div className="p-2 sticky top-0 bg-gray-900 border-b border-gray-800 z-10">
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex items-center justify-between">
                                                <h4 className="text-gray-200 text-sm font-medium truncate">{voice.title}</h4>
                                                <span className="text-[10px] bg-gray-800 text-gray-400 px-1.5 rounded">{t(`gallery.filters.${voice.category}`)}</span>
                                            </div>
                ...
                <div className="w-full md:w-2/3 flex flex-col gap-2 h-full">
                    ...
                    <textarea 
                        className="w-full h-full min-h-[200px] bg-gray-900 border border-gray-700 rounded-xl p-4 text-gray-200 focus:outline-none focus:border-indigo-500 font-sans text-lg leading-relaxed resize-none"
      ...
      {showSidebar && (
        <div className="w-full md:w-1/3 lg:w-1/4 bg-gray-900 border-l border-gray-800 flex flex-col h-full overflow-hidden animate-in fade-in slide-in-from-right-10 duration-300">
            {/* Console Log */}
            <div className="h-1/2 p-4 border-b border-gray-800 flex flex-col">
                <div className="flex items-center gap-2 text-gray-400 mb-2">
                    <Terminal className="w-4 h-4" />
                    <span className="text-xs font-mono font-bold uppercase">{t('workspace.systemLog')}</span>
                </div>
                <div className="flex-1 bg-black rounded-lg p-3 overflow-y-auto font-mono text-xs space-y-1">
                    {logs.map((log, i) => (
                        <div key={i} className="text-emerald-500/80 break-words">{log}</div>
                    ))}
            ...
            <div className="h-1/2 p-4 flex flex-col overflow-hidden">
                <div className="flex items-center justify-between text-gray-400 mb-4">
                    ...
                    <button className="p-1 hover:text-white" onClick={() => setHistory([])}><Trash2 className="w-3 h-3" /></button>
                </div>
                
                <div className="flex-1 overflow-y-auto space-y-3">
                    {history.length === 0 && (
                        <div className="text-center text-gray-600 text-xs py-10">{t('workspace.noGenerations')}</div>
                    )}
                    {history.map((item) => (
                        <div key={item.id} className="bg-gray-800 rounded-lg p-3 hover:bg-gray-750 transition-colors group relative">
                            <div className="flex justify-between items-start mb-2">
                                <span className="text-xs text-gray-500 font-mono">{new Date(item.timestamp).toLocaleTimeString()}</span>
                                <div className="flex items-center gap-2">
                                    {item.isPublished && <span className="text-[10px] bg-indigo-900/50 text-indigo-300 px-1 rounded border border-indigo-800">{t('workspace.published')}</span>}
                                    <span className="text-xs bg-gray-700 text-gray-300 px-1.5 rounded">{item.duration}s</span>
                                </div>
                            </div>
                            <p className="text-sm text-gray-300 line-clamp-2 mb-3">{item.text}</p>
                            <div className="flex gap-2">
                                <button className="flex-1 bg-indigo-600/20 hover:bg-indigo-600 text-indigo-400 hover:text-white text-xs py-1.5 rounded flex items-center justify-center gap-1 transition-colors">
                                ...
                                <button 
                                    ...
                                    className={`p-1.5 rounded transition-colors ${item.isPublished ? 'bg-green-900/30 text-green-500' : 'bg-gray-700 hover:bg-gray-600 text-gray-300'}`}
                                >
