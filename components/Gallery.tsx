    ...
    <div className="p-6 h-full overflow-y-auto">
      ...
      <div className="flex flex-col gap-6 mb-8">
        ...
        <div className="flex gap-4 border-b border-gray-800">
             <button
                ...
                className={`pb-3 px-2 font-bold text-sm transition-colors relative ${activeTab === 'voices' ? 'text-indigo-400' : 'text-gray-500 hover:text-gray-300'}`}
             >
                {t('gallery.tabs.voices')}
                {activeTab === 'voices' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-indigo-500 rounded-t-full" />}
             </button>
             <button
                onClick={() => setActiveTab('audiobooks')}
                className={`pb-3 px-2 font-bold text-sm transition-colors relative ${activeTab === 'audiobooks' ? 'text-indigo-400' : 'text-gray-500 hover:text-gray-300'}`}
             >
                {t('gallery.tabs.audiobooks')}
                {activeTab === 'audiobooks' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-indigo-500 rounded-t-full" />}
             </button>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4">
            ...
            <input 
                ...
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                value={searchTerm}
      ...
      {activeTab === 'voices' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <div key={voice.id} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden hover:border-gray-500 transition-all group">
                <div className="relative h-40 bg-gray-700">
                <img src={voice.imageUrl} alt={voice.title} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                <div className="absolute inset-0 bg-gradient-to-t from-gray-900 to-transparent opacity-90" />
                
                <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
                    <div>
                    <h3 className="text-lg font-bold text-white truncate">{voice.title}</h3>
                    <p className="text-xs text-indigo-400 font-mono">{t('gallery.by')}: {voice.author}</p>
                    </div>
                    <button 
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    <span className="px-2 py-0.5 rounded-full bg-indigo-900/50 text-indigo-300 text-xs border border-indigo-700/50 font-bold">
                        {t(`gallery.filters.${voice.category}`)}
                    </span>
                    {voice.tags.slice(0, 2).map(tag => (
                    <span key={tag} className="px-2 py-0.5 rounded-full bg-gray-700 text-gray-300 text-xs border border-gray-600">
                        #{tag}
                    </span>
                    ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <button 
                    className="flex items-center justify-center gap-2 bg-gray-700 hover:bg-gray-600 text-white py-2 rounded-lg text-sm font-medium transition-colors"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              ...
              {filteredWorks.map((work) => (
                  <div key={work.id} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden hover:border-gray-500 transition-all flex flex-col">
                       {/* Audiobook Cover */}
                       <div className={`h-48 bg-gradient-to-br ${work.coverColor} p-6 relative flex flex-col justify-between`}>
                           <div className="flex justify-between items-start">
                               ...
                               {work.visibility === 'private' ? (
                                   <div className="bg-black/40 px-2 py-1 rounded text-[10px] text-white flex items-center gap-1">
                                       <Lock className="w-3 h-3" /> {t('gallery.private')}
                                   </div>
                               ) : (
                                   <div className="bg-emerald-500/20 px-2 py-1 rounded text-[10px] text-emerald-100 border border-emerald-500/30 flex items-center gap-1">
                           ...
                           <div>
                               <h3 className="text-xl font-serif font-bold text-white line-clamp-2 leading-tight">{work.title}</h3>
                               <p className="text-sm text-white/70 mt-1">{t('gallery.by')} {work.author}</p>
                           </div>

                           <button 
                                onClick={() => handlePlayToggle(work.id)}
                                className="absolute bottom-[-20px] right-6 w-12 h-12 rounded-full bg-indigo-500 hover:bg-indigo-400 flex items-center justify-center shadow-lg transition-transform hover:scale-105 border-4 border-gray-800"
                            >
                       ...
                       <div className="p-6 pt-8 flex-1 flex flex-col">
                            <p className="text-gray-400 text-sm line-clamp-3 mb-4 flex-grow">{work.description}</p>
                            
                            <div className="flex items-center justify-between text-xs text-gray-500 mt-auto border-t border-gray-700/50 pt-4">
                                <div className="flex items-center gap-1">
